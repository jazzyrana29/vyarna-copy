...
export class DiceService {
  ...
  async startGame(
    ...
    const serverSeedHash = createHash('sha256')
      .update(serverSeed)
      .digest('hex');
    const clientSeed = startDiceGameDto.clientSeed || randomBytes(8).toString('hex');
    ...
    try {
      ...
      this.logger.info(`Game ${saved.sessionId} created`, traceId, 'startGame', LogStreamLevel.ProdStandard);
      return { sessionId: saved.sessionId, serverSeedHash: saved.serverSeedHash };
    } catch (error: any) {
      this.logger.error(`Error starting game: ${error.message}`, traceId, 'startGame', LogStreamLevel.DebugHeavy);
      throw error;
    }
  }

  async roll(rollDiceDto: RollDiceDto, traceId: string): Promise<RollResultDto> {
    this.logger.debug(
    ...
    try {
      game = await this.diceRepo.findOne({ where: { sessionId: rollDiceDto.sessionId } });
    } catch (error: any) {
      this.logger.error(`Error fetching game: ${error.message}`, traceId, 'roll', LogStreamLevel.DebugHeavy);
      throw error;
    }
    ...
    const multiplier = this.computeMultiplier(rollDiceDto.target, rollDiceDto.rollOver);
    ...
    try {
      ...
      this.logger.info(`Game ${game.sessionId} rolled`, traceId, 'roll', LogStreamLevel.ProdStandard);
    } catch (error: any) {
      this.logger.error(`Error saving roll: ${error.message}`, traceId, 'roll', LogStreamLevel.DebugHeavy);
      throw error;
    }
  ...
  async cashout(
    ...
    let game: DiceGame | null = null;
    try {
      game = await this.diceRepo.findOne({ where: { sessionId: cashoutDiceGameDto.sessionId } });
    } catch (error: any) {
      this.logger.error(`Error fetching game: ${error.message}`, traceId, 'cashout', LogStreamLevel.DebugHeavy);
      throw error;
    }
    if (!game) throw new NotFoundException('Game not found');
    game.isFinished = true;
    try {
      await this.diceRepo.save(game);
      this.logger.info(`Game ${game.sessionId} cashed out`, traceId, 'cashout', LogStreamLevel.ProdStandard);
    } catch (error: any) {
      this.logger.error(`Error saving game: ${error.message}`, traceId, 'cashout', LogStreamLevel.DebugHeavy);
      throw error;
    }
    return {
      sessionId: game.sessionId,
  ...
  async getConfig(
    ...
    this.logger.debug('Fetching config', traceId, 'getConfig', LogStreamLevel.DebugLight);
    return this.config;
  }
  ...
  async getProvablyFair(
    ...
    this.logger.debug(`Provably fair for ${diceProvablyFairRequestDto.sessionId}`, traceId, 'getProvablyFair', LogStreamLevel.DebugLight);
    let game: DiceGame | null = null;
    try {
      game = await this.diceRepo.findOne({ where: { sessionId: diceProvablyFairRequestDto.sessionId } });
    } catch (error: any) {
      this.logger.error(`Error fetching game: ${error.message}`, traceId, 'getProvablyFair', LogStreamLevel.DebugHeavy);
      throw error;
    }
    if (!game) throw new NotFoundException('Game not found');
    if (!game.isFinished)
      throw new BadRequestException('Game not finished yet');
    this.logger.info(`Provably fair data returned for ${game.sessionId}`, traceId, 'getProvablyFair', LogStreamLevel.ProdStandard);
    return {
      serverSeedHash: game.serverSeedHash,
  ...
  async autoBet(
    ...
    const strategy =
      ...
      (autoBetSettingsDto.martingale ? AutoBetStrategy.MARTINGALE : AutoBetStrategy.MANUAL);
    ...
    const adjustOnWin = (bet: number): number => {
      switch (strategy) {
        ...
        case AutoBetStrategy.DALEM:
          return bet + autoBetSettingsDto.onWinPct * autoBetSettingsDto.initialBet;
        default:
          return bet;
      }
    };
    ...
    const adjustOnLoss = (bet: number): number => {
      switch (strategy) {
        ...
        case AutoBetStrategy.DELAYED_MARTINGALE:
          return consecutiveLosses >= 3 ? bet * (1 + autoBetSettingsDto.onLossPct / 100) : bet;
        case AutoBetStrategy.DALEM:
          return Math.max(
