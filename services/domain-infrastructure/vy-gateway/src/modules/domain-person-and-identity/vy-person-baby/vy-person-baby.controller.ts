...
@ApiTags('vy-person-baby')
@Controller('vy-person-baby')
export class PersonBabyController {
  private logger = getLoggerConfig(PersonBabyController.name);

  constructor(
    private readonly personBabyKafkaService: PersonBabyKafkaService,
  ) {
    this.logger.debug(
      `${PersonBabyController.name} initialized`,
      '',
      'constructor',
