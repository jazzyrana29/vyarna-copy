...
@ApiTags('vy-sales-store')
@Controller('vy-sales-store')
export class SalesStoreController {
  private logger = getLoggerConfig(SalesStoreController.name);

  constructor(
    private readonly personBabyKafkaService: SalesStoreKafkaService,
  ) {
    this.logger.debug(
      `${SalesStoreController.name} initialized`,
      '',
      'constructor',
