...
export async function socketCreateSession(roomId: string, dto: CreateSessionDto): Promise<SessionDto> {
  ...
  return new Promise((resolve, reject) => {
    ...
    socketSvc.on<SessionDto>(KT_CREATE_SESSION_RESULT, (data) => { cleanup(); resolve(data); });
    socketSvc.on<string>(KT_CREATE_SESSION_ERROR, (msg) => { cleanup(); reject(new Error(msg)); });
...
export async function socketUpdateSession(roomId: string, dto: UpdateSessionDto): Promise<SessionDto> {
  ...
  return new Promise((resolve, reject) => {
    ...
    socketSvc.joinRoom(roomId);
    const cleanup = () => socketSvc.disconnect();
    socketSvc.on<SessionDto>(KT_UPDATE_SESSION_RESULT, (data) => { cleanup(); resolve(data); });
    socketSvc.on<string>(KT_UPDATE_SESSION_ERROR, (msg) => { cleanup(); reject(new Error(msg)); });
...
export async function socketGetSession(roomId: string, dto: GetOneSessionDto): Promise<SessionDto> {
  ...
  return new Promise((resolve, reject) => {
    ...
    socketSvc.joinRoom(roomId);
    const cleanup = () => socketSvc.disconnect();
    socketSvc.on<SessionDto>(KT_GET_SESSION_RESULT, (data) => { cleanup(); resolve(data); });
    socketSvc.on<string>(KT_GET_SESSION_ERROR, (msg) => { cleanup(); reject(new Error(msg)); });
